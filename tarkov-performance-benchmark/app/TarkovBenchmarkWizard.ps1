...
$btnCollect.Add_Click({
    try {
        ...
        Invoke-BenchmarkScript -Name "read-eft-settings.ps1" | Set-Content -LiteralPath $State.SettingsJsonPath -Encoding UTF8
        Invoke-BenchmarkScript -Name "collect-system-info.ps1" | Set-Content -LiteralPath $State.SystemJsonPath -Encoding UTF8
...
$btnReadLogs.Add_Click({
    try {
        ...
        $contextJson = Invoke-BenchmarkScript -Name "read-raid-context.ps1"
        ...
        if ($context.mode -eq "online") {
            $cmbMode.Text = "PvP online BSG"
        }
        elseif ($context.mode -eq "offline/local") {
            $cmbMode.Text = "Offline practice"
        }
        ...
        Add-Log "Read raid context from logs: map=$($context.map), mode=$($context.mode), confidence=$($context.confidence)"
        Set-Status "Raid context loaded from logs. Check weather/time/activity manually."
    }
    catch {
        Add-Log "ERROR: $($_.Exception.Message)"
...
$btnBuildRun.Add_Click({
    try {
        ...
        Invoke-BenchmarkScript -Name "build-run-json.ps1" -Arguments $args | Out-Null
