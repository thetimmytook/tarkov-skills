...
- Both Store products are publicly published. The current public versions are Benchmark `1.0.3.0` and Toolkit `1.0.0.0`.
- Build and test MSIX packages in GitHub Actions. Partner Center upload remains manual until the Store deployment pipeline is deliberately introduced; every update still passes Microsoft certification before publication.
...
- For a closed Store flight, use a private audience containing personal Microsoft accounts, not work or school accounts. Public production submissions publish automatically after certification unless a deliberate publishing hold is required.
