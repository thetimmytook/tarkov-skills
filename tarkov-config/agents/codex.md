...
- If the user changes the target FPS or quality/performance tradeoff, run the analyzer with `-Goal`, `-TargetFpsMin`, `-QualityPreference`, and `-SaveGoal` so `memory/current-goal.json` is updated.
