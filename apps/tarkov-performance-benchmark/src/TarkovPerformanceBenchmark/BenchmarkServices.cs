...
internal static class TarkovState
{
...
internal sealed class SettingsReader
{
    public (object Settings, List<string> Warnings) Read()
    {
        ...
        foreach (var item in new[] { ("graphics", "Graphics.ini"), ("postfx", "PostFx.ini"), ("game", "Game.ini") })
        {
            ...
            catch (Exception ex) { result[item.Item1] = null; warnings.Add($"{item.Item2} could not be read: {ex.Message}"); }
        }
...
internal sealed class SystemInfoCollector
{
    public (object System, List<string> Warnings) Collect()
    {
        ...
        var gpu = QueryMany("SELECT Name,DriverVersion,CurrentHorizontalResolution,CurrentVerticalResolution FROM Win32_VideoController", o => new { name = Text(o, "Name"), driver_version = Text(o, "DriverVersion"), current_resolution = Resolution(o) }, warnings);
        ...
        var pagefiles = QueryMany("SELECT AllocatedBaseSize,CurrentUsage,PeakUsage FROM Win32_PageFileUsage", o => new { allocated_gb = Math.Round(Number(o, "AllocatedBaseSize") / 1024d, 2), current_usage_gb = Math.Round(Number(o, "CurrentUsage") / 1024d, 2), peak_usage_gb = Math.Round(Number(o, "PeakUsage") / 1024d, 2) }, warnings);
        var totalRam = modules.Sum(m => Convert.ToDouble(m.GetType().GetProperty("capacity_gb")!.GetValue(m), CultureInfo.InvariantCulture));
        return (new { os, cpu, gpu, ram = new { total_gb = Math.Round(totalRam, 2), modules }, pagefile = new { files = pagefiles } }, warnings);
    }

    private static object? QueryOne(string query, Func<ManagementBaseObject, object> map, List<string> warnings) { try { using var searcher = new ManagementObjectSearcher(query); return searcher.Get().Cast<ManagementBaseObject>().Select(map).FirstOrDefault(); } catch (Exception ex) { warnings.Add($"System information was partially unavailable: {ex.Message}"); return null; } }
    private static List<object> QueryMany(string query, Func<ManagementBaseObject, object> map, List<string> warnings) { try { using var searcher = new ManagementObjectSearcher(query); return searcher.Get().Cast<ManagementBaseObject>().Select(map).ToList(); } catch (Exception ex) { warnings.Add($"System information was partially unavailable: {ex.Message}"); return []; } }
...
internal sealed class RaidLogReader
{
